...
Formula.parse = function (input) {
  ...
  let actions = [
    ...
    { re : /[%]/, action : function (s, i, j) { return new Mod(); }},
  ...
  */
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token instanceof Const) {
      ...
      while (op_stack.peek() instanceof Binop && op_stack.peek().priority >= token.priority) {
        //pop the operator from the stack and reduce it to the output
          op = op_stack.pop();
          output_stack.reduce(op);
      }
  ...
  if (output_stack.length !== 1) {
    //console.log(output_stack.length);
